...
namespace ThyniusWeb.Server.Controllers
{
    ...
    public class UserTextController : Controller
    {
        ...
        public async Task<IActionResult> DeleteUserText(int textId)
        {
            ...
            if(callingUser.Id != userText.OwnerId)
            {
                return Unauthorized();
            }
        ...
        public async Task<string> GetSubmittmentInfo(int textId)
        {
            ...
            Assignment assignment = await _assignmentRepository.GetByIdAsync(text.Id);
            Group group = await _groupRepository.GetAssignmentGroup(assignment);
            if(group.ManagerId != group.ManagerId)
            {
        ...
        public async Task<string> GetAssignmentUserTexts(int assignmentId)
        {
            ...
            if (!(assignmentGroup.ManagerId == callingUser.Id))
            {
                HttpContext.Response.StatusCode = 401;
                return null;
            }
