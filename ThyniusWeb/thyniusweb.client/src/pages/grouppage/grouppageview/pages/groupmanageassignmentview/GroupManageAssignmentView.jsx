    ...
    <div className='flex flex-col gap-5 h-[100%] w-[100%] mt-[70px] md:mt-[96px] items-between'>
      <div className='flex flex-col gap-3 md:flex-row w-[80%] justify-between'>
        <div>
          ...
          <Input className='w-full sm:w-[280px] text-base' defaultValue={state.name} onChange={updateName} variant="faded" radius='sm' type='name' label={t("Assignment name")}></Input>
        </div>
        <div>
      ...
      <div>
        ...
        <Textarea variant="faded" defaultValue={state.description} onChange={updateDescription} label={t("Description")} placeholder={t("Description of the assignment")} className='w-full sm:w-[400px]'></Textarea>
      </div>
      <div>
      <div className="px-3">
        <p className={props.hasClickedSubmit && errors?.imageError ? "text-red-600 text-xl pb-3" : "text-xl pb-3"}>{t("Assignment Image")}</p>
        <input type="file"  accept=".jpg,.img" onChange={updateImage}></input>
        {state.image ? <LoadingImage img={state.image} className='mt-5 w-[100px] h-[100px] sm:w-[130px] sm:h-[130px]'></LoadingImage> : <div className='mt-5 w-[100px] h-[100px] sm:w-[130px] sm:h-[130px]'></div>}
      </div>
      </div>
